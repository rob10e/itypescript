...
var Configuration = /** @class */ (function () {
    ...
    Object.defineProperty(Configuration.prototype, "config", {
        ...
        get: function () {
            ...
            var typesdir = path.join(rootDir, "node_modules", "@types");
            var typepaths = [];
            if (fs.existsSync(typesdir)) {
                typepaths = fs.readdirSync(typesdir).map(function (dir) {
                    return "/// <reference path=\"" + path.join(typesdir, dir, "index.d.ts") + "\"/>";
                });
            }
            else {
                typepaths = [];
            }
            var snapshot = typepaths;
